...
from .implementations import PytorchImplementation
from .utils import drop_init, sort_parameters


def em_algorithm(
    X, Q, n_init=10, iterations=100, implementation=PytorchImplementation()
):
    ...
    for i in range(iterations):
        for k in range(n_init):
            ...
            previous_ll[k] = ll
        print(
            f"After EM iteration {i}/{iterations} : Mean log likelihood ({n_init} paths) {np.mean(previous_ll):5f}...",
            end="",
        )
